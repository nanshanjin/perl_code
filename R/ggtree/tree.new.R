...
g<-ggtree(raxml,size=.1,layout="rectangular",color="black")+geom_tiplab(size=.7,align=F,linesize=.2)
if (!is.null(opt$raxml)){
...
png(paste(opt$outfile,".rectangular.tree.png",sep=""),height=800*3,width=800*3,res=72*3)
print(g)
dev.off()
g<-NULL;

g<-ggtree(raxml,size=.1,layout="circular",color="black")+geom_tiplab2(size=.7,align=F,linesize=.2,aes(angle=angle))
if (!is.null(opt$raxml)){
g<-g+geom_text2(size=1,color="black",aes(subset=!isTip, label=bootstrap,color="black"))
}
if(!is.null(opt$group)){
...
dev.off()
png(paste(opt$outfile,".circular.tree.png",sep=""),height=800*3,width=800*3,res=72*3)
print(g)
