...
for (i in 1:dim(dat)[1]){
    #chr_pos <- data[i,1]
    x <- as.numeric(dat[i,5:8])
    dim(x) <- c(2,2)
    a <- fisher.test(x,alternative="two.sided")
    p_orig <- a$p
    or <- as.matrix(a$estimate)[1]
    u   <- rbind(u, data.frame(CHR =dat[i,1] ,POS=dat[i,2] ,REF=dat[i,3] ,ALT=dat[i,4] ,Alt_case=dat[i,5],Alt_control=dat[i,6],Ref_case=dat[i,7],Ref_control=dat[i,8], Pvalue=p_orig, OR=or))
  }
pvalue <- u[,9]
